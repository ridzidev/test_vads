...
SELECT
    ...
    mi.estimate_price,
    CASE
        WHEN mi.estimate_price < 50000 THEN @discount_price_1
        WHEN mi.estimate_price BETWEEN 50000 AND 1500000 THEN @discount_price_2
        ELSE @discount_price_3
    END AS discount,
    mi.estimate_price - (mi.estimate_price * CASE
        WHEN mi.estimate_price < 50000 THEN @discount_price_1
        WHEN mi.estimate_price BETWEEN 50000 AND 1500000 THEN @discount_price_2
        ELSE @discount_price_3
    END) AS fix_price
FROM user u
JOIN master_items mi ON u.id = mi.id_name;
```

#### Result Example
| name              | items                  | estimate_price | discount | fix_price |
|-------------------|------------------------|----------------|----------|-----------|
| jonatan christie | Lampu bohlam LED 20 WATT | 20.000        | 0.02    | 19600    |
| jonatan christie | Mouse wireless logitech | 175.000       | 0.035   | 168875   |
| los blancos      | Snack                   | 50.000        | 0.035   | 48250    |
| los blancos      | TV Plasma               | 2.500.000     | 0.05    | 2375000  |
| mawang           | Headphone               | 750.000       | 0.035   | 723750   |
| mawang           | Lemari Besi             | 1.500.000     | 0.05    | 1425000  |
